...
plotMA(res_D18_A28_D22_A28, ylim=c(1,4))

#2. Compare gene expression between devtemp treatment groups at A28
...
#makek a list of which genes in our comparisons of interest are differentially expressed (List of DEGs)
degs_D18_A28_D22_A28 <- rownames(res_D18_A28_D22_A28[res_D18_A28_D22_A28$padj < 0.05,])

#3. compare gene expresstion between dev temp groups at A33
...
degs_D18_A33_D22_A33 <- rownames(res_D18_A33_D22_A33[res_D18_A33_D22_A33$padj < 0.05,])
...
#We see that the effect of devTemp decreases when finalTemp increases 
...
length(intersect(intersect(degs_D18_A28_D22_A28,degs_D18_A33_D22_A33),degs_D18_BASE_D22_BASE)) #23
...
296 - 107 - 29 + 23 #183 genes diff expressed uniquely at A28 at for 18 vs 22
...
myEuler <- euler(c("BASE"=1807,"A28"=183,"A33"=28,"BASE&A28"=84,"BASE&A33"=21, "A33&A28"=6, "BASE&A28&A33"=23))
myEuler
...
res_df28 <- res_df28 %>%
  mutate(fill= case_when(
    padj.18<0.05 & stat.18 <0 ~ "turquoise2", # genes that are significant and upregulated in experimental env
